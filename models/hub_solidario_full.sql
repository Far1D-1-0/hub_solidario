-- ╔══════════════════════════════════════════════════════════════════════════════╗
-- ║  Hub Solidario — base de datos completa                                    ║
-- ║  Ejecutar desde cero: mysql -u root -p < models/hub_solidario_full.sql     ║
-- ╚══════════════════════════════════════════════════════════════════════════════╝
...
-- ══════════════════════════════════════════════════════════════════════════════
-- SEED: PROYECTOS (15 de catálogo + 1 showcase)
-- ══════════════════════════════════════════════════════════════════════════════
...
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='ALIMENTACION'), (SELECT id_usuario FROM Usuario WHERE email='maria.gonzalez@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='EDUCACION'), (SELECT id_usuario FROM Usuario WHERE email='carlos.ramirez@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='MEDIO_AMBIENTE'), (SELECT id_usuario FROM Usuario WHERE email='ana.martinez@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='SALUD'), (SELECT id_usuario FROM Usuario WHERE email='luis.torres@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='CONSTRUCCION'), (SELECT id_usuario FROM Usuario WHERE email='roberto.sanchez@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='TECNOLOGIA'), (SELECT id_usuario FROM Usuario WHERE email='fernanda.lopez@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='MEDIO_AMBIENTE'), (SELECT id_usuario FROM Usuario WHERE email='diana.herrera@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='SALUD'), (SELECT id_usuario FROM Usuario WHERE email='claudia.vega@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='EDUCACION'), (SELECT id_usuario FROM Usuario WHERE email='miguel.ruiz@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='ARTE'), (SELECT id_usuario FROM Usuario WHERE email='sofia.ramos@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='MEDIO_AMBIENTE'), (SELECT id_usuario FROM Usuario WHERE email='tomas.guerrero@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='EDUCACION'), (SELECT id_usuario FROM Usuario WHERE email='patricia.morales@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='ALIMENTACION'), (SELECT id_usuario FROM Usuario WHERE email='rosa.fuentes@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='MEDIO_AMBIENTE'), (SELECT id_usuario FROM Usuario WHERE email='jorge.espinoza@hub.mx')
),
(
  ...
  @activo, (SELECT id_categoria FROM Categoria WHERE codigo='SALUD'), (SELECT id_usuario FROM Usuario WHERE email='valeria.soto@hub.mx')
);
...
-- KPIs del primer proyecto (Banco de Alimentos)
...
-- ══════════════════════════════════════════════════════════════════════════════
-- SEED: PROYECTO SHOWCASE (Centro Comunitario Integral)
-- ══════════════════════════════════════════════════════════════════════════════
...
SET @ruta_csv = CONCAT('archivos_usuario/', @lider, '/showcase_indicadores_jun2024.csv');

INSERT INTO `ArchivoUsuario` (ruta_archivo, id_usuario, id_tipo_archivo)
  SELECT @ruta_csv, @lider,
         (SELECT id_tipo_archivo FROM TipoArchivo WHERE codigo = 'DATASET')
  WHERE NOT EXISTS (
    SELECT 1 FROM ArchivoUsuario WHERE ruta_archivo = @ruta_csv
  );

SET @archivo = (SELECT id_archivo_usuario FROM ArchivoUsuario WHERE ruta_archivo = @ruta_csv LIMIT 1);
...
INSERT INTO `Evento` (nombre, descripcion, fecha_realizacion, fecha_publicacion, id_usuario_creador, id_proyecto, id_estado_evento) VALUES
  ('Reunión de Voluntarios — Junio 2026',    'Sesión mensual de coordinación con todos los voluntarios activos. Se revisarán los avances del mes y se planificarán las actividades del siguiente periodo.',                                                                                        '2026-06-05 17:00:00', '2026-05-28 00:00:00', @lider,   @pid,       @ev_pasado),
  ('Taller de Habilidades Digitales',        'Capacitación en herramientas digitales básicas: correo electrónico, búsqueda segura en internet y uso de hojas de cálculo. Dirigido a adultos mayores de la comunidad. Cupo limitado a 20 personas.',                                          '2026-06-15 10:00:00', '2026-06-01 00:00:00', @lider,   @pid,       @ev_publicado),
  ('Jornada Comunitaria de Limpieza',        'Actividad de embellecimiento del parque principal de la Colonia Solidaria. Se dotará de materiales a todos los participantes. Punto de encuentro: entrada principal del parque a las 8:00 hrs.',                                               '2026-06-22 08:00:00', '2026-06-08 00:00:00', @lider,   @pid,       @ev_programado),
  ('Noche de Cine Comunitario',              'Proyección de película bajo las estrellas en el patio del centro. Entrada libre para toda la familia. Se proyectará "Coco" con temática de identidad y comunidad.',                                                                             '2026-06-28 19:30:00', '2026-06-10 00:00:00', @lider,   @pid,       @ev_programado),
  ('Feria de Emprendimiento Social',         'Exposición de proyectos comunitarios, stands de emprendedores locales y talleres rápidos de finanzas personales e innovación social. Evento abierto al público sin costo de entrada.',                                                        '2026-07-12 09:00:00', '2026-06-20 00:00:00', @lider,   @pid,       @ev_programado),
  ('Taller de Finanzas para el Hogar',       'Aprende a elaborar un presupuesto familiar, reducir deudas y ahorrar de forma efectiva. Impartido por voluntarios del área financiera. Sesión de 3 horas con materiales incluidos.',                                                          '2026-07-19 11:00:00', '2026-06-25 00:00:00', @lider,   @pid,       @ev_programado),
  ('Ceremonia de Graduación — Beneficiarios 2026', 'Reconocimiento a los beneficiarios que completaron los programas de capacitación del año. Contará con la presencia de autoridades locales, familias y medios de comunicación.',                                                         '2026-08-15 18:00:00', '2026-07-01 00:00:00', @lider,   @pid,       @ev_programado),
  ('Siembra Colectiva de Temporada',         'Jornada de siembra de hortalizas de temporada en los huertos comunitarios. Aprende técnicas de cultivo orgánico y llévate plántulas para tu hogar. No se requiere experiencia previa.',                                                      '2026-06-21 08:00:00', '2026-06-07 00:00:00', @admin_u, @huertos_p, @ev_programado),
  ('Cosecha y Trueque de Productos',         'Primera cosecha de la temporada y mercado de trueque entre vecinos. Trae tus excedentes de jardín y llévate algo nuevo. Evento familiar con música en vivo.',                                                                                 '2026-08-08 09:00:00', '2026-07-15 00:00:00', @admin_u, @huertos_p, @ev_programado),
  ('Jornada Médica Gratuita',                'Consultas médicas generales, toma de presión arterial, glucosa y orientación nutricional sin costo. Atención por orden de llegada. Cupos disponibles: 80 personas.',                                                                          '2026-07-05 09:00:00', '2026-06-15 00:00:00', @admin_u, @brigadas_p,@ev_programado),
  ('Entrega de Reconocimientos Académicos',  'Ceremonia de premiación a los estudiantes destacados del ciclo escolar que participaron en el programa de tutorías. Asistencia abierta a padres de familia.',                                                                                  '2026-07-26 16:00:00', '2026-07-01 00:00:00', @admin_u, @tutorias_p,@ev_programado);
